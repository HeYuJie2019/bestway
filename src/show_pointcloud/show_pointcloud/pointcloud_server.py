    ...
    def run_websocket_server(self):
        ...
        async def handler(websocket):
            ...
            try:
                ...
                self.get_logger().debug(f'Client {client_address} disconnected normally')
            except websockets.exceptions.InvalidMessage as e:
                self.get_logger().warning(f'Invalid WebSocket message from {client_address} (可能是端口扫描或不兼容客户端)')
            except websockets.exceptions.InvalidHeader as e:
                self.get_logger().warning(f'Invalid WebSocket header from {client_address} (可能是HTTP请求到WebSocket端口)')
            except Exception as e:
                # 减少错误日志的详细程度，避免刷屏
                if "EOFError" in str(e) or "connection closed" in str(e).lower():
                    self.get_logger().debug(f'Client {client_address} connection error: {str(e)}')
                else:
                    self.get_logger().error(f'WebSocket error with {client_address}: {str(e)}')
            finally:
                self.connected_clients.discard(websocket)
                self.get_logger().debug(f'Client {client_address} cleaned up')
        ...
        async def start_server():
            ...
            self.get_logger().info(f'Available access addresses:')
            for ip in local_ips:
                self.get_logger().info(f'  ws://{ip}:{self.ws_port}')
            self.get_logger().info(f'  ws://localhost:{self.ws_port}')
